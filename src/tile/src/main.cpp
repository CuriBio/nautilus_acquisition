...
class VideoWriter {
    ...
    public:
        VideoWriter(std::filesystem::path outdir, std::string codec, int framerate, size_t width, size_t height) :
...
int main(int argc, char* argv[]) {
    ...
    VideoWriter w(outdir, codec, 10, 2400, 1200);
    w.Initialize();
    ...
    /*     outTiff.Write16(static_cast<uint16_t*>(data)); */
    /* }; */

    PostProcess::AutoTile(indir, outdir, frames, rows, cols, width, height, false, true, true, writerFn);
    w.close();
    //outTiff.Close();
